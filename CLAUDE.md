...
| **reconstruction.py end_reach** | `_reconstruct_reach_end_reach` and `reactive.py` both use `n_up > 1 OR n_down > 1`. The historical bug in `flow_verification.py` was fixed in commit `d019194`. |
...
| **path_freq=0/-9999 on ghost reaches only** | All 23K nodata `path_freq` values are ghost reaches (type=6). Connected non-ghost reaches have valid values. Issue #16 closed. |
| **POM lint findings (mostly resolved)** | N013 CL-node misallocations: 89K→311 (99.7% fixed). T017 dist_out jumps: 701→557. All investigation issues (#188–#196) closed. See `docs/technical/pom_requests_summary.md` for current gate status. |
...
| **Flow correction disabled (0.0.9)** | ALL 810 flow-corrected reaches reverted to v17b topology after discovering a scoring tautology (`slope_from_upstream = -slope_from_downstream`). Topology now matches v17b exactly (0 diff). Fixed scorer + cross-run oscillation guard exist in PR #221 but flow correction is disabled pending team review. `v17c_flow_corrections` table is empty. |
