# Revert OC Reach 51111300061 Split — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Undo an incomplete `break_reaches()` call that split reach 51111300061 (OC) into 4 segments, leaving 2 orphan reach_ids, stale geometry, and wrong topology.

**Architecture:** Python script using DuckDB connections to both v17b (read-only source) and v17c (write target). Restores centerlines and nodes from v17b, deletes spurious data, rebuilds geometry, verifies with lint checks.

**Tech Stack:** Python, DuckDB, spatial extension

**Key constraint:** DB files live at `/Users/jakegearon/projects/SWORD/data/duckdb/`, NOT in the worktree.

---

### Task 1: Write the revert script with verification

**Files:**
- Create: `scripts/maintenance/revert_oc_reach_split.py`
- Test: manual verification via the script's `--dry-run` and `--verify` modes
...
def precondition_checks(con_c: duckdb.DuckDBPyConnection, con_b: duckdb.DuckDBPyConnection):
...
def dry_run(con_c: duckdb.DuckDBPyConnection, con_b: duckdb.DuckDBPyConnection):
    ...
    print(f"  DELETE {current_cls} current CLs for {ORIGINAL_REACH}, replace with 586 from v17b")
    print(f"  DELETE {current_nodes} current nodes for {ORIGINAL_REACH}, insert 85 missing from v17b")
    ...
    print(f"  UPDATE reach {ORIGINAL_REACH} metadata from v17b (x, y, bbox, reach_length, n_nodes)")
...
def execute(con_c: duckdb.DuckDBPyConnection, con_b: duckdb.DuckDBPyConnection):
    ...
    # Get v17b centerline columns that exist in v17c
    v17b_cl_cols = [r[0] for r in con_b.execute("DESCRIBE centerlines").fetchall()]
    ...
    v17b_cls = con_b.execute(
        f"SELECT {cols_str} FROM centerlines WHERE reach_id = {ORIGINAL_REACH} AND region = '{REGION}'"
    ).fetchdf()
    con_c.execute(f"INSERT INTO centerlines ({cols_str}) SELECT * FROM v17b_cls")
    print(f"  Inserted {len(v17b_cls)} v17b centerlines")

    # --- Step 4: Restore missing nodes from v17b ---
    print("Step 4: Restore missing nodes from v17b...")

    # Get node_ids currently in v17c for this reach (14 nodes with SWOT data — keep them)
    existing_nids = set(
        con_c.execute(
            f"SELECT node_id FROM nodes WHERE reach_id = {ORIGINAL_REACH} AND region = '{REGION}'"
        ).fetchdf()["node_id"].tolist()
    )

    # Get all v17b node columns that exist in v17c
    v17b_node_cols = [r[0] for r in con_b.execute("DESCRIBE nodes").fetchall()]
    ...
    # Insert only the missing v17b nodes (85 of 99)
    v17b_nodes = con_b.execute(
        f"SELECT {node_cols_str} FROM nodes WHERE reach_id = {ORIGINAL_REACH} AND region = '{REGION}'"
    ).fetchdf()
    missing_nodes = v17b_nodes[~v17b_nodes["node_id"].isin(existing_nids)]
    con_c.execute(f"INSERT INTO nodes ({node_cols_str}) SELECT * FROM missing_nodes")
    print(f"  Kept {len(existing_nids)} existing nodes (have SWOT data)")
    print(f"  Inserted {len(missing_nodes)} missing v17b nodes")

    # --- Step 5: Delete spurious reach ---
    print("Step 5: Delete spurious reach and topology...")

    # Delete reach_swot_orbits and reach_ice_flags for spurious reach if they exist
    ...
    v17b_meta = con_b.execute(
        f"SELECT x, y, x_min, x_max, y_min, y_max, reach_length, n_nodes FROM reaches WHERE reach_id = {ORIGINAL_REACH}"
    ).fetchdf().iloc[0]
    con_c.execute(f"""
        UPDATE reaches SET
            x = {v17b_meta['x']},
            y = {v17b_meta['y']},
            x_min = {v17b_meta['x_min']},
            x_max = {v17b_meta['x_max']},
            y_min = {v17b_meta['y_min']},
            y_max = {v17b_meta['y_max']},
            reach_length = {v17b_meta['reach_length']},
            n_nodes = {v17b_meta['n_nodes']}
        WHERE reach_id = {ORIGINAL_REACH}
    """)
    print(f"  Updated x, y, bbox, reach_length, n_nodes from v17b")
...
def verify(con_c: duckdb.DuckDBPyConnection):
    ...
    n = con_c.execute(
        f"SELECT COUNT(*) FROM centerlines c LEFT JOIN reaches r ON c.reach_id = r.reach_id WHERE r.reach_id IS NULL"
    ).fetchone()[0]
    status = "PASS" if n == 0 else "FAIL"
    ...
    # 3. No orphan nodes
    n = con_c.execute(
        f"SELECT COUNT(*) FROM nodes n LEFT JOIN reaches r ON n.reach_id = r.reach_id WHERE r.reach_id IS NULL"
    ).fetchone()[0]
    status = "PASS" if n == 0 else "FAIL"
    ...
    # 9. Neighbor counts
    for rid, col, expected in [(NEIGHBOR_051, "n_rch_up", 1), (NEIGHBOR_071, "n_rch_down", 1)]:
        val = con_c.execute(
...
**Step 2: Run dry-run to verify preconditions**

Run: `cd /Users/jakegearon/projects/SWORD && python scripts/maintenance/revert_oc_reach_split.py --dry-run`

Expected: All precondition checks pass, shows summary of changes.

**Step 3: Execute the revert**

Run: `cd /Users/jakegearon/projects/SWORD && python scripts/maintenance/revert_oc_reach_split.py --execute`

Expected: All 9 steps complete without error.

**Step 4: Verify the revert**

Run: `cd /Users/jakegearon/projects/SWORD && python scripts/maintenance/revert_oc_reach_split.py --verify`

Expected: All 11 checks PASS.

**Step 5: Run C005 lint check to confirm fix**

Run: `cd /Users/jakegearon/projects/SWORD && python -c "
import duckdb
from sword_duckdb.lint.checks.classification import check_centerline_reach_distance
con = duckdb.connect('data/duckdb/sword_v17c.duckdb', read_only=True)
con.execute('INSTALL spatial; LOAD spatial;')
r = check_centerline_reach_distance(con, region='OC')
print(f'C005 OC: passed={r.passed}, issues={r.issues_found}')
con.close()
"`

Expected: `C005 OC: passed=True, issues=0`

**Step 6: Commit**

```bash
git add scripts/maintenance/revert_oc_reach_split.py
git commit -m "fix: revert incomplete reach split for OC 51111300061

An ad-hoc break_reaches() call split this 19.7km reach into 4 segments
but only created 2 of 4 reach records, leaving 434 orphan centerlines,
73 orphan nodes, and stale geometry. Reverts to v17b state while
preserving v17c-only columns and SWOT percentile data on existing nodes."
```

---

### Task 2: Handle the C005 false positive for NA reach 83242100011

This reach is 34.8km long and the 8.3km centroid offset is geometric (line centroid vs point-average centroid). Not a data error. No code change needed — just document it.

**Step 1: Verify it's still the only remaining C005 issue**
...
con = duckdb.connect('data/duckdb/sword_v17c.duckdb', read_only=True)
con.execute('INSTALL spatial; LOAD spatial;')
r = check_centerline_reach_distance(con)
print(f'C005 global: passed={r.passed}, issues={r.issues_found}')
if not r.details.empty:
    print(r.details[['reach_id', 'region', 'dist_m', 'cl_count']].to_string())
con.close()
"`

Expected: 1 issue — reach 83242100011 (NA), ~8304m, 396 CLs. This is a 34.8km reach where geometry centroid naturally diverges from arithmetic mean of CL points. Acceptable.
